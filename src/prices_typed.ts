...
function createApp(database: Database) {
  ...
  function isHoliday(date: Date, plainDate: Temporal.PlainDate = convertDate(date) ) {
    const holidays = database.getHolidays();
    for (let row of holidays) {
      let holiday2 = convertDate(new Date(row.holiday));
      if (
        plainDate &&
        convertDate(date).year === holiday2.year &&
        convertDate(date).month === holiday2.month &&
          convertDate(date).day === holiday2.day
      ) {
        return true;
      }
    }
  ...
  function convertDate(date: Date):Temporal.PlainDate { return Temporal.PlainDate.from(date.toISOString().replace("Z",""))}
